...
let EW_NAME = "eyeworld"; in {
    programs.zsh = let EW_ROOT = "$HOME/dev/roles/${EW_NAME}"; in {
        ...
        '';
        shellAliases = {
            eyeworld-ssh-breachserver = "ssh -i ${EW_ROOT}/secrets/.ssh/id_rsa-breach-server djgr@54.160.62.225";
            eyeworld-sshfs-breachserver = "sudo sshfs -o debug,allow_other,IdentityFile=${EW_ROOT}/secrets/.ssh/id_rsa-breach-server,reconnect,ServerAliveInterval=15,ServerAliveCountMax=3 djgr@54.160.62.225:/ /Volumes/EyeWorld/Breaches/";
